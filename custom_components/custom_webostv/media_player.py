...
async def async_setup_entry(
    ...
    async_add_entities([CustomLgWebOSMediaPlayerEntity(wrapper, name, sources, unique_id)])


_T = TypeVar("_T", bound="CustomLgWebOSMediaPlayerEntity")
_P = ParamSpec("_P")
...
class CustomLgWebOSMediaPlayerEntity(RestoreEntity, MediaPlayerEntity):
    ...
    def _update_sources(self) -> None:
        ...
        # custom >>
        if self._client.channels is not None:
            for channel in self._client.channels:
                self._source_list[channel['channelName']] = {'label': channel['channelName'], 'title': MediaType.CHANNEL, 'id': channel['channelNumber']}
        # << custom
    ...
    async def async_select_source(self, source: str) -> None:
        ...
        if (source_dict := self._source_list.get(source)) is None:
            ...
            return
        # custom >>
        if source_dict['title'] == MediaType.CHANNEL:
            await self.async_play_media(MediaType.CHANNEL, source_dict['label'])
            return
        # << custom
    ...
    async def async_play_media(
        ...
        if media_type == MediaType.CHANNEL:
            ...
            # custom >>
            if self._current_source != 'LiveTV':
                await self.async_select_source('LiveTV')
            # << custom
